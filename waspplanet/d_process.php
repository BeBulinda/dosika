...
<div class="container">
    <div class="news-paper">
        <div class="sign_up text-center">
            <form id="frm_subcription" action="d_process.php" method="post">
...
if(isset($_POST['submit'])){
  ...
  /*  $find_download = mysql_query("SELECT * FROM videos WHERE url='$sessionx'");
    $row_download = mysql_fetch_assoc($find_download);
    $v_id = $row_download['v_id'];
    $u_id = $row_download['u_id'];
    $insert = mysql_query("INSERT INTO downloads(token, v_id, url, used, number, u_id) VALUE('$token', '$v_id', '$sessionx', '1', 'NULL', '$u_id')");
    */
    ...
    $row_d = mysql_fetch_assoc($url_check);
             if($token_s){
                 if($token_s['used']==0){
                     /* Download Documenting */
                    $display_u = "SELECT * FROM videos WHERE url = '$sessionx'";
